...
bool bfs()
{
	...
	while(!q.empty())
	{
		...
		if(maze[r][c][direct][0]) q.push(make_pair(Node(r-1, c, 0), w.first));
		if(maze[r][c][direct][1]) q.push(make_pair(Node(r+1, c, 1), w.first));
		if(maze[r][c][direct][2]) q.push(make_pair(Node(r, c-1, 2), w.first));
		if(maze[r][c][direct][3]) q.push(make_pair(Node(r, c+1, 3), w.first));
	}
	return false;
}
...
int main()
{
	while(cin>>name && name != "END")
	{
		...
		snode.d = -1;		//这里很重要。因为会回到开始位置，但是下一步方向不一样。
