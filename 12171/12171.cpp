...
int grid[maxd][maxd][maxd];		//0:空	//1：铜
	...
	while(!q.empty())
	{
        	...
        	for(int i=0; i<6; i++)
		{
			int tx, ty, tz;
            		...
            		ty = y + dy[i];
            		tz = z + dz[i];
            		if(tx<0 || tx>=an[0]-1 || ty<0 || ty>=an[1]-1 || tz<0 || tz>=an[2]-1)
                		continue;
            		if(grid[tx][ty][tz] == 1)
			{
            			if (i < 2)
...
int main()
{
	...
	while(T--)
	{
		...
		for(int i = 0; i < 3; i++)
		{
			...
			an[i] = unique(ad[i], ad[i]+2*n+1)-ad[i];
			ad[i][an[i]++] = EDGE;
		}
		...
		for(int c = 0; c < n; c++)
		{
			...
			x0 = lower_bound(ad[0], ad[0]+an[0], cube[c][0])-ad[0];
			x1 = lower_bound(ad[0], ad[0]+an[0], cube[c][3])-ad[0];
			y0 = lower_bound(ad[1], ad[1]+an[1], cube[c][1])-ad[1];
			y1 = lower_bound(ad[1], ad[1]+an[1], cube[c][4])-ad[1];
			z0 = lower_bound(ad[2], ad[2]+an[2], cube[c][2])-ad[2];
			z1 = lower_bound(ad[2], ad[2]+an[2], cube[c][5])-ad[2];
			for(int i = x0; i < x1; i++)
				for(int j = y0; j < y1; j++)
					for(int k = z0; k < z1; k++)
						grid[i][j][k] = 1;
