 ...
 * 	      	    =很大的数。每个改动判断是否唯一解，还要进行40000次运算.
 * 	      思路：1.ida。这是最直接的思路，直觉判断会超时。
 * 	      	    2.先根据当前谜语，得到所有等式，然后根据等式形式，反向推导唯一解谜语。推导
 * 	      	    方法是固定固定某几位*,看看结果是否为唯一。这道题就变成了集合查找问题。这种思
 * 	      	    路是错误的：如果改动不是固定*,而是把一个数字变成另一个数字，或者变成一
 * 	      	    个*。可能会导致错误答案，因为对转移做了限制。如8 ** 8*应当改为6 ** 8*
